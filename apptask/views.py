...
def summary(request):
    ...
    join = Record.objects.raw('SELECT apptask_site.id, apptask_site.site_name, SUM(apptask_record.a_record) AS a_record, SUM(apptask_record.b_record) AS b_record FROM apptask_record INNER JOIN apptask_site ON (apptask_record.site_id = apptask_site.id) GROUP BY apptask_site.id ORDER BY apptask_site.id')
